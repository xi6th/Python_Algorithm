class Node:
    """
        An object for storing signle node
        Nodels two attributes - data and the link to the next node in the last
    """
    data = None
    next_node = None
...
class LinkedList:
    ...
    def search(self,key):
        """
            ...
            Return the node `None` if not found
    ...
    def remove(self, key):
        """
            Removes Node contaning data that matches the key
            Returns the node or None if key soesn't exit
            Takes O(n)
        """
        current = self.head
        previous = None
    ...
    def __repr__(self):
        ...
        """
            Returns a string reprensentation of the list
            Takes O(n) time
        """
        nodes = []
        current = self.head
